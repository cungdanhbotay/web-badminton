    ...
    <main>
      ...
      <div class="container">
        <div class="row">
          <div class="col">
            <div class="card-body">
              <h3>Statistics on the number of products sold</h3>
            </div>
            <div id="ketqua"></div>
            <canvas id="chartOfobjChartThongKeLoaiSanPham"></canvas>
            <button class="btn btn-outline-primary" id="refreshThongKeLoaiSanPham">Take data</button>
          </div>
          <div class="w-100"></div>
          <div class="col">
            <div class="card-body">
            ...
            <button class="btn btn-outline-primary" id="ThongKeSanPhamBanChayNhat">Take data</button>
          </div>
    ...
    function renderChartThongKeLoaiSanPham() {
      $.ajax({
        ...
        success: function(response) {
          ...
          var myData = [];
          $(data).each(function() {
            myLabels.push(("Remaining statistics:"));
            myData.push(this.soluongtonkho);
            myLabels.push(("Sales statistics:"));
  ...
  $(document).ready(function() {
    function top_sale_amount() {
      $.ajax('/website_tmdt/admin/api/desc_amount_product.php', {
        ...
        error: function() {
          var htmlString = `<h1>Can't handle </h1>`;
          $('#baocaoSanPham_SoLuong').html(htmlString);
        }
      });
    }
